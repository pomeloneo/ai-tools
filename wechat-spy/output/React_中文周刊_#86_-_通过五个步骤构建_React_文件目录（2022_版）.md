...
**React Admin v4.0：一个为 B2B 应用打造的管理系统框架** — 该框架用于构建基于浏览器的管理系统（后端支持 REST、GraphQL 等，你也可以写一个自己的适配层）。如果你想在线查看，这里有 一个示例。React Admin 采用 MIT 许可证，如果需要支持的话也可以使用专业版。这是它的 **GitHu****b  仓库地址**。
