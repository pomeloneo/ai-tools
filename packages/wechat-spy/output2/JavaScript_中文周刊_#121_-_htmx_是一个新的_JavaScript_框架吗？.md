# JavaScript 中文周刊 #121 - htmx 是一个新的 JavaScript 框架吗？

> 原文链接: http://mp.weixin.qq.com/s?__biz=MzIzOTkwMjM0OQ==&mid=2247526066&idx=1&sn=59453964699ee4dd0a46f8e18d218003&chksm=e9212d50de56a44603c2d1a684246787f5e7535555ba8623776b7e2ebc96c2bea5d31fcb77aa#rd
...
**根据脚本语言进行 CSS 媒体查询** —— Chrome（和 Edge）刚刚添加了一个新的媒体查询（例如 `@media (scripting: none)`），可以根据脚本语言的可用性定义样式。这个功能现在在所有主要的浏览器中都得到支持，这是 Chromium 追赶的一个罕见案例！你可以在 **这里** 查看它在所有主要浏览器中的支持情况。
...
**remoteStorage：跨浏览器/设备持久化存储** —— 将 `localStorage` API 与远程服务器结合起来，以实现在各个浏览器和设备之间持久化存储数据——基本上就像是远程的 `localStorage`。这是 **服务器部分** 的链接。
...
-   **file-type v19.0** – 从 `Buffer`、`Uint8Array` 或 `ArrayBuffer` 中检测文件类型。
